  ...
  while (pib != nullptr) {
    if (pib->block_name.compare(0, 6, "output") == 0) {
      ...
      if (op.dt > 0.0 || op.dcycle > 0) {  // only add output if dt > 0 or dycle > 0
        ...
        if (PARTICLES && op.block_name == "outputp") {
          ...
          if (op.file_type == "tab") {
            ...
            msg << "### FATAL ERROR in Outputs constructor" << std::endl
                ...
                << "' in output block '" << op.block_name << "' for particles"
                << std::endl;
            ATHENA_ERROR(msg);
          }
        } else {
...
void OutputType::LoadOutputData(MeshBlock *pmb) {
  ...
  if (ContainVariable(output_params.variable, "D") ||
    ...
    pod->type = "SCALARS";
    pod->name = "dens";
    if (pmb != nullptr) pod->data.InitWithShallowSlice(phyd->u, 4, IDN, 1);
  ...
  if (ContainVariable(output_params.variable, "d") ||
    ...
    pod->type = "SCALARS";
    pod->name = "rho";
    if (pmb != nullptr) pod->data.InitWithShallowSlice(phyd->w, 4, IDN, 1);
    AppendOutputDataNode(pod);
    num_vars_++;
  }
  ...
  if (NON_BAROTROPIC_EOS) {
    if (ContainVariable(output_params.variable, "E") ||
      ...
      if(porb->orbital_advection_defined
        ...
        if (pmb != nullptr) pod->data.InitWithShallowSlice(porb->u_orb, 4, IEN, 1);
      } else {
        if (pmb != nullptr) pod->data.InitWithShallowSlice(phyd->u, 4, IEN, 1);
      }
      AppendOutputDataNode(pod);
      num_vars_++;
    }
    ...
    if (ContainVariable(output_params.variable, "p") ||
      ...
      pod->type = "SCALARS";
      pod->name = "press";
      if (pmb != nullptr) pod->data.InitWithShallowSlice(phyd->w, 4, IPR, 1);
      AppendOutputDataNode(pod);
      num_vars_++;
    }
  }
  ...
  if (ContainVariable(output_params.variable, "m") ||
    ...
    if(porb->orbital_advection_defined
       && !output_params.orbital_system_output) {
      porb->ConvertOrbitalSystem(phyd->w, phyd->u, OrbitalTransform::cons);
      if (pmb != nullptr) pod->data.InitWithShallowSlice(porb->u_orb, 4, IM1, 3);
    } else {
      if (pmb != nullptr) pod->data.InitWithShallowSlice(phyd->u, 4, IM1, 3);
    }
    AppendOutputDataNode(pod);
    num_vars_ += 3;
    if (output_params.cartesian_vector) {
      ...
      pod->type = "VECTORS";
      pod->name = "mom_xyz";
      if (pmb != nullptr) pod->data.NewAthenaArray(3, phyd->u.GetDim3(),
                                                   phyd->u.GetDim2(),
                                                   phyd->u.GetDim1());
  ...
  if (ContainVariable(output_params.variable, "m1")) {
    ...
    pod->type = "SCALARS";
    pod->name = "mom1";
    if (pmb != nullptr) pod->data.InitWithShallowSlice(phyd->u, 4, IM1, 1);
    AppendOutputDataNode(pod);
    num_vars_++;
  }
  if (ContainVariable(output_params.variable, "m2")) {
    ...
    if(porb->orbital_advection_defined
      ...
      porb->ConvertOrbitalSystem(phyd->w, phyd->u, OrbitalTransform::cons);
      if (pmb != nullptr) pod->data.InitWithShallowSlice(porb->u_orb, 4, IM2, 1);
    } else {
      if (pmb != nullptr) pod->data.InitWithShallowSlice(phyd->u, 4, IM2, 1);
    }
    AppendOutputDataNode(pod);
    num_vars_++;
  }
  if (ContainVariable(output_params.variable, "m3")) {
    ...
    if(porb->orbital_advection_defined
      ...
      porb->ConvertOrbitalSystem(phyd->w, phyd->u, OrbitalTransform::cons);
      if (pmb != nullptr) pod->data.InitWithShallowSlice(porb->u_orb, 4, IM3, 1);
    } else {
      if (pmb != nullptr) pod->data.InitWithShallowSlice(phyd->u, 4, IM3, 1);
    }
    AppendOutputDataNode(pod);
    num_vars_++;
  }
  ...
  if (ContainVariable(output_params.variable, "v") ||
    ...
    if(porb->orbital_advection_defined
      ...
      if (pmb != nullptr) pod->data.InitWithShallowSlice(porb->w_orb, 4, IVX, 3);
    } else {
      if (pmb != nullptr) pod->data.InitWithShallowSlice(phyd->w, 4, IVX, 3);
    }
    AppendOutputDataNode(pod);
    num_vars_ += 3;
    if (output_params.cartesian_vector) {
  ...
  if (ContainVariable(output_params.variable, "vx") ||
    ...
    pod->type = "SCALARS";
    pod->name = "vel1";
    if (pmb != nullptr) pod->data.InitWithShallowSlice(phyd->w, 4, IVX, 1);
    AppendOutputDataNode(pod);
    num_vars_++;
  }
  if (ContainVariable(output_params.variable, "vy") ||
    ...
    if(porb->orbital_advection_defined
       ...
       && porb->orbital_direction == 1) {
      porb->ConvertOrbitalSystem(phyd->w, phyd->u, OrbitalTransform::prim);
      if (pmb != nullptr) pod->data.InitWithShallowSlice(porb->w_orb, 4, IVY, 1);
    } else {
      if (pmb != nullptr) pod->data.InitWithShallowSlice(phyd->w, 4, IVY, 1);
    }
    AppendOutputDataNode(pod);
    num_vars_++;
  }
  if (ContainVariable(output_params.variable, "vz") ||
    ...
    if(porb->orbital_advection_defined
       ...
       && porb->orbital_direction == 2) {
      porb->ConvertOrbitalSystem(phyd->w, phyd->u, OrbitalTransform::prim);
      if (pmb != nullptr) pod->data.InitWithShallowSlice(porb->w_orb, 4, IVZ, 1);
    } else {
      if (pmb != nullptr) pod->data.InitWithShallowSlice(phyd->w, 4, IVZ, 1);
    }
    AppendOutputDataNode(pod);
    num_vars_++;
  }
  ...
  if (SELF_GRAVITY_ENABLED) {
    if (ContainVariable(output_params.variable, "phi") ||
      ...
      pod->type = "SCALARS";
      pod->name = "Phi";
      if (pmb != nullptr) pod->data.InitWithShallowSlice(pgrav->phi, 4, 0, 1);
      AppendOutputDataNode(pod);
      num_vars_++;
    }
  ...
  if (NSCALARS > 0) {
    ...
    for (int n=0; n<NSCALARS; n++) {
      ...
      if (ContainVariable(output_params.variable, scalar_name_cons) ||
        ...
        if (pmb != nullptr) pod->data.InitWithShallowSlice(psclr->s, 4, n, 1);
        AppendOutputDataNode(pod);
        num_vars_++;
      }
      if (ContainVariable(output_params.variable, scalar_name_prim) ||
        ...
        if (pmb != nullptr) pod->data.InitWithShallowSlice(psclr->r, 4, n, 1);
        AppendOutputDataNode(pod);
        num_vars_++;
      }
    }
  ...
  if ((PARTICLES) && !(MONTE_CARLO_ENABLED)) {
  ...
  if (MAGNETIC_FIELDS_ENABLED) {
    ...
    if (ContainVariable(output_params.variable, "bcc") ||
      ...
      pod->type = "VECTORS";
      pod->name = "Bcc";
      if (pmb != nullptr) pod->data.InitWithShallowSlice(pfld->bcc, 4, IB1, 3);
      AppendOutputDataNode(pod);
      num_vars_ += 3;
      if (output_params.cartesian_vector) {
        AthenaArray<Real> src;
        ...
        pod->type = "VECTORS";
        pod->name = "Bcc_xyz";
        if (pmb != nullptr) pod->data.NewAthenaArray(3, pfld->bcc.GetDim3(),
                                                     pfld->bcc.GetDim2(),
                                                     pfld->bcc.GetDim1());
        CalculateCartesianVector(src, pod->data, pmb->pcoord);
        AppendOutputDataNode(pod);
    ...
    if (ContainVariable(output_params.variable, "bcc1")) {
      ...
      pod->type = "SCALARS";
      pod->name = "Bcc1";
      if (pmb != nullptr) pod->data.InitWithShallowSlice(pfld->bcc, 4, IB1, 1);
      AppendOutputDataNode(pod);
      num_vars_++;
    }
    if (ContainVariable(output_params.variable, "bcc2")) {
      ...
      pod->type = "SCALARS";
      pod->name = "Bcc2";
      if (pmb != nullptr) pod->data.InitWithShallowSlice(pfld->bcc, 4, IB2, 1);
      AppendOutputDataNode(pod);
      num_vars_++;
    }
    if (ContainVariable(output_params.variable, "bcc3")) {
      ...
      pod->type = "SCALARS";
      pod->name = "Bcc3";
      if (pmb != nullptr) pod->data.InitWithShallowSlice(pfld->bcc, 4, IB3, 1);
      AppendOutputDataNode(pod);
      num_vars_++;
    }
    ...
    if (ContainVariable(output_params.variable, "b1")
      ...
      pod->type = "SCALARS";
      pod->name = "B1";
      if (pmb != nullptr) pod->data.InitWithShallowSlice(pfld->b.x1f, 4, 0, 1);
      AppendOutputDataNode(pod);
      num_vars_++;
    }
    if (ContainVariable(output_params.variable, "b2")
      ...
      pod->type = "SCALARS";
      pod->name = "B2";
      if (pmb != nullptr) pod->data.InitWithShallowSlice(pfld->b.x2f, 4, 0, 1);
      AppendOutputDataNode(pod);
      num_vars_++;
    }
    if (ContainVariable(output_params.variable, "b3")
      ...
      pod->type = "SCALARS";
      pod->name = "B3";
      if (pmb != nullptr) pod->data.InitWithShallowSlice(pfld->b.x3f, 4, 0, 1);
      AppendOutputDataNode(pod);
      num_vars_++;
    }
  ...
  if (MONTE_CARLO_ENABLED) {
    ...
    if (output_params.variable.compare("mclab") == 0) {
      ...
      pod->type = "SCALARS";
      pod->name = "tgas";
      if (pmb != nullptr) pod->data.InitWithShallowSlice(pmcb->tgas,4,0,1);
      AppendOutputDataNode(pod);
      num_vars_++;
    }
    // monte carlo density
    if (output_params.variable.compare("mclab") == 0) {
      ...
      pod->type = "SCALARS";
      pod->name = "rho";
      if (pmb != nullptr) pod->data.InitWithShallowSlice(pmcb->rho,4,0,1);
      AppendOutputDataNode(pod);
      num_vars_++;
    }
...
void Outputs::MakeOutputs(Mesh *pm, ParameterInput *pin, bool wtflag) {
  ...
  if (pout_ != nullptr && pout_->CheckTimer(pm)) {
    pout_->WriteOutputFile(pm);
    pout_->SetNextOutput(pin);
  }
}
