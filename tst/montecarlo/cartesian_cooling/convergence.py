...
def cooling(tgas,rho,Ermc,hnu,kapj,emin,emax,scatflag=True):
    """
    Computes the cooling assuming free-free emission and absorption, and Compton scattering
    """
    kb = 1.3806580e-16
    c = 2.9979e10
    me = 9.1094e-28
    ...
    if (scatflag):
        ...
        kappa_es = 0.
    #print('kapes, kapJ:',kappa_es,kapj)
    eta, kappap = eta_kappap_ff(rho,tgas,emin,emax) #includes factor of 4pi

    cool = -eta + c*kapj*rho*Ermc - (c*rho*kappa_es*4.*kb*tgas*Ermc / (me*c**2)) + (c*rho*kappa_es*hnu*Ermc / (me*c**2))
