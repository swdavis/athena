...
used rather than stored as a cell-centre quantity.
...
of `|u.u + 1|` evaluated at the photon's *zone centre* -- the frame `ComputeTransformations` and
...
```
  nx1   64  photon-frame |u.u+1| 1.1102e-15   zone-centre 8.8818e-16
  nx1  128  photon-frame |u.u+1| 1.1102e-15   zone-centre 6.6613e-16
  nx1  256  photon-frame |u.u+1| 1.3323e-15   zone-centre 8.8818e-16
PASS: both frames are unit timelike where they are used
```

The split is documented at `UpdateMoments`: the moments are zone averages, so one frame per zone
...
`O(dx dg)` -- the same order as the zone averaging itself.
...
from reading `vel` to calling `FluidFourVelocity` at the zone centre was confirmed **bitwise**
identical (`rel 0.000e+00`, 17 digits, every zone) with a temporary comparison inside
...
**Still deferred, by choice:** the cell-centre tetrad applied to a wavevector carried at the photon
without parallel transport (the cached `acon` correction), and refreshing opacity within a zone.
...
The two remaining consumers that legitimately want a *zone-centre* four-velocity --
`ComputeTransformations` and `ComovingFrameMatrix`, which build the per-zone moment frames -- call
`FluidFourVelocity` at the zone centre instead of reading `vel`. That was the one real risk in this
...
every zone of a Kerr-Schild run, so the moment frames are unchanged.
...
`MonteCarloBlock::vel` stores a contravariant four-velocity built at the **cell centre**, normalized
...
offset. Measured on a Kerr-Schild grid (a = 0.9375, 128 logarithmic radial zones from 1.2M to 100M,
64 in theta, photon half a zone off centre):
...
| GR + general pusher | contravariant `u^mu` normalized at cell centre | fails at O(dx dg) | **yes** |
...
- `UpdateMoments` stays on the cell-centre frames. Out of scope, with a guard test.
...
1. `MRWAcceleration` read `pmcb->vel(m, i3, i2, i1)`. `vel` is allocated
   ...
   indexes another zone's memory and runs off the end of the array once `i1` reaches 4. It also
...
Nonzero, and **first order in the zone width**, which is the signature of an offset error rather
than anything else. The peak sits at r = 3.124 on the coarse grid; the zone edges are at
`2.0 + n*0.2812`, so 3.125 is exactly a zone boundary -- the point farthest from a zone centre.
After Phase 3 this column should collapse to roundoff at every resolution.

Two things surfaced while building this:

- **`vel` was zero in ghost zones. Fixed.** Every fluid-derived array was filled over active
  zones only, but a photon can hold ghost indices while it waits to be handed to the neighbouring
...
Runs need a large `<montecarlo>/checkmove`: with `varystep` the step is a fraction of a zone, so
...
`uprim` holds the primitive relative velocity `uu^i` per zone in GR, filled beside `vel` in
...
is zero and the same formula returns the normal observer at `x` -- exact, since no zone-centre
quantity enters.
...
GR only. Rebuilds `u^mu` from the zone's stored primitives with the metric at `x`, using the same
...
`vel`'s GR branch stores primitives; `ComputeTransformations` reconstructs a cell-centre `u^mu`
...
position, the comoving moments at the zone centre. The cell-centre-tetrad question and the cached
